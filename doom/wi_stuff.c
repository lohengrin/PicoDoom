...
#define SP_TIMEX		16
// Logical (320x200-space) constant -- mixing physical SCREENHEIGHT with a
// logical literal here would double-scale once fed to V_DrawPatch/WI_drawTime.
  ...
  patch_t*	c[] )
{
	...
	// left/top/right/bottom are all logical (320x200-space, from the
	// lnodes[] table and raw patch dimensions) -- compare against
	// logical BASE_WIDTH/BASE_HEIGHT, not physical SCREENWIDTH/HEIGHT.
	if (left >= 0
	    && right < BASE_WIDTH
	    && top >= 0
	    && bottom < BASE_HEIGHT)
